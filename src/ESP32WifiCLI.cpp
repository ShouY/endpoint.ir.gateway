...
void ESP32WifiCLI::deleteNetwork(String ssid) {
  ...
  while (cfg.isKey(String(getNetKeyName(net) + "_ssid").c_str())) {
    ...
    if (!dropped && ssid_.equals(ssid)) {
      ...
      int default_net = cfg.getInt("default_net", 1);
      if (net - 1 == default_net && net_count > 1)
        cfg.putInt("default_net", net_count - 1);
      continue;
    }
    if (dropped) {
...
void ESP32WifiCLI::wifiAPConnect(bool save) {
  ...
  while (WiFi.status() != WL_CONNECTED &&
         retry++ < 20) {  // M5Atom will connect automatically
    delay(1000);
    Serial.print(".");
  }
  Serial.println();
...
void _scanNetworks(String opts) { wcli.scan(); }

void _printHelp(String opts) { wcli.printHelp(); }
...
void _connect(String opts) { wcli.connect(); }

void _disconnect(String opts) { wcli.disconnect(); }
...
void ESP32WifiCLI::begin(Stream* stream, String app) {
  ...
  term->add("mode", &_setMode,
            "\tset the default operation single/multi AP (slow)");
  ...
  term->add("delete", &_deleteNetwork,
            "\tremove saved WiFi network by SSID\r\n");
}
